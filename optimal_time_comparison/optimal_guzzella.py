...
def inverse_dynamics_2R(q, qd, qdd):
    ...
    # Mass matrix M(q)
    M11 = I1 + m1*d1**2 + I2 + m2*d2**2 + m2*L1**2 + 2*m2*L1*L2 * np.cos(q2)
...
# q_i = np.array([0, 0])  #### TEST A0
# q_f = np.array([0.98, 0]) 

# q_i = np.array([0, 0])  #### TEST A1
# q_f = np.array([1, 0]) 

q_i = np.array([0, 0])  #### TEST B0
q_f = np.array([0.785, -2*np.pi])

# q_i = np.array([0, 0])  #### TEST B1
# q_f = np.array([np.pi, -2*np.pi])

###################### CHOOSE AND GENERATE WAYPOINTS  ######################
initial_waypoints = generate_waypoints(q_i, q_f , 1) 
# initial_waypoints = generate_waypoints(q_i, q_f , 2)  
# initial_waypoints = generate_waypoints(q_i, q_f , 3)  
# initial_waypoints = generate_waypoints(q_i, q_f , 4) 
# initial_waypoints = generate_waypoints(q_i, q_f , 5) 
# initial_waypoints = generate_waypoints(q_i, q_f, 10) 

####################### TORQUE CONSTRAINTS DEFINITION #######################

tau_max = np.array([[-25, 25], [-9, 9]])
...
# Function to calculate trajectory duration given waypoints
def compute_trajectory_time(waypoints_flat):
    ...
    #instance = algo.TOPPRA([pc_vel, pc_acc, pc_tau], path)
...
#                       seed=None, target_accept_rate=0.5, stepwise_factor=0.5)
...
#plt.plot(qs_sample[:, 0], qs_sample[:, 1], label="Optimized Joint Path")
#plt.scatter([q_i[0], optimized_waypoint[0], q_f[0]], [q_i[1], optimized_waypoint[1], q_f[1]], color='r', label="Optimized Path")
# Plot settings
