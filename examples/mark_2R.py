...
def inverse_kinematics_2R(x, y, L1, L2):
    ...
    s2=-np.abs(np.sqrt(1-c2**2)) # positive conf
    #s2=-np.abs(np.sqrt(1-c2**2)) # negative conf
...
# QUESTION MARK
def generate_question_arc(x0, y0, a, b):
    points = []
...
waypoints_fist_1 = generate_line_with_slope(slope=1, intercept=2.5, x_start=-1.3, x_end=-1)
waypoints_line1 = generate_vertical_line(x0 = -1  , y0 = 1.5) #finisce in (-1,0.5)
intra1 = generate_line_with_slope(slope=7, intercept=7.5, x_start=-1, x_end=-0.9)
waypoints_second_1 = generate_line_with_slope(slope=1, intercept=2.1, x_start=-0.9, x_end=-0.6)
waypoints_line2 = generate_vertical_line(x0 = -0.6, y0 = 1.5)
intra2 = generate_line_with_slope(slope=0.625, intercept=0.875, x_start=-0.6, x_end=0.2)
waypoints_circle = generate_ellipse(x0 = -0.1, y0 = 1, a = 0.3 , b= 0.5)
intra3 = generate_line_with_slope(slope=2.5, intercept=0.5, x_start=0.2, x_end=0.4)
waypoints_line3 = generate_vertical_line(x0= 0.4, y0=1.5)
waypoints_line4 = generate_horizontal_line(x0=0.4 , y0=0.5)
intra4 = generate_vertical_line_reversed(x0= 0.9, y0= 0.5)
waypoints_question = generate_question_arc(x0=0.9, y0=1.2, a=0.3, b=0.3)
waypoints_slope_question = generate_line_with_slope(slope=2/3, intercept=0.2, x_start=1.5, x_end=1.2)
waypoints_vertical_question = generate_vertical_question(x0=1.2, y0=1)
intra5 = generate_line_with_slope(slope=-2, intercept=3.1, x_start=1.2, x_end=1.275)
waypoints_circle_question = generate_circle(x0=1.2, y0=0.55, radius=0.075)
...
for i in range(len(ps_sample_first_1)):
    q1, q2 = inverse_kinematics_2R(ps_sample_first_1[i,0], ps_sample_first_1[i, 1], L1, L2)
    joint_way_pts_first_1.append([q1, q2])
...
for i in range(len(ps_sample1)):
    q1, q2 = inverse_kinematics_2R(ps_sample1[i,0], ps_sample1[i, 1], L1, L2)
    joint_way_pts1.append([q1, q2])
...
for i in range(len(ps_intra1)):
    q1, q2 = inverse_kinematics_2R(ps_intra1[i,0], ps_intra1[i, 1], L1, L2)
    joint_way_intra1.append([q1, q2])
...
for i in range(len(ps_sample_second_1)):
    q1, q2 = inverse_kinematics_2R(ps_sample_second_1[i,0], ps_sample_second_1[i, 1], L1, L2)
    joint_way_pts_second_1.append([q1, q2])
...
for i in range(len(ps_sample2)):
    q1, q2 = inverse_kinematics_2R(ps_sample2[i,0], ps_sample2[i, 1], L1, L2)
    joint_way_pts2.append([q1, q2])
...
for i in range(len(ps_intra2)):
    q1, q2 = inverse_kinematics_2R(ps_intra2[i,0], ps_intra2[i, 1], L1, L2)
    joint_way_intra2.append([q1, q2])
...
for i in range(len(ps_sample3)):
    q1, q2 = inverse_kinematics_2R(ps_sample3[i,0], ps_sample3[i, 1], L1, L2)
    joint_way_pts3.append([q1, q2])
...
for i in range(len(ps_intra3)):
    q1, q2 = inverse_kinematics_2R(ps_intra3[i,0], ps_intra3[i, 1], L1, L2)
    joint_way_intra3.append([q1, q2])
...
for i in range(len(ps_sample4)):
    q1, q2 = inverse_kinematics_2R(ps_sample4[i,0], ps_sample4[i, 1], L1, L2)
    joint_way_pts4.append([q1, q2])
...
for i in range(len(ps_sample5)):
    q1, q2 = inverse_kinematics_2R(ps_sample5[i,0], ps_sample5[i, 1], L1, L2)
    joint_way_pts5.append([q1, q2])
...
for i in range(len(ps_intra4)):
    q1, q2 = inverse_kinematics_2R(ps_intra4[i,0], ps_intra4[i, 1], L1, L2)
    joint_way_intra4.append([q1, q2])
...
joint_way_pts6 = []
for i in range(len(ps_sample1)):
    q1, q2 = inverse_kinematics_2R(ps_sample6[i,0], ps_sample6[i, 1], L1, L2)
    joint_way_pts6.append([q1, q2])
...
for i in range(len(ps_sample7)):
    q1, q2 = inverse_kinematics_2R(ps_sample7[i,0], ps_sample7[i, 1], L1, L2)
    joint_way_pts7.append([q1, q2])
...
for i in range(len(ps_sample8)):
    q1, q2 = inverse_kinematics_2R(ps_sample8[i,0], ps_sample8[i, 1], L1, L2)
    joint_way_pts8.append([q1, q2])
...
for i in range(len(ps_intra5)):
    q1, q2 = inverse_kinematics_2R(ps_intra5[i,0], ps_intra5[i, 1], L1, L2)
    joint_way_intra5.append([q1, q2])
...
for i in range(len(ps_sample9)):
    q1, q2 = inverse_kinematics_2R(ps_sample9[i,0], ps_sample9[i, 1], L1, L2)
    joint_way_pts9.append([q1, q2])
...
+ jnt_traj7.duration + jnt_traj8.duration  + jnt_intra5.duration
+ jnt_traj9.duration

print("TOTAL TRAJECTORY TIME IS: ", tot_time)

### CONCATENATE
# qs_samples = [qs_sample1, qs_sample_intra1, qs_sample2, qs_sample_intra2, qs_sample3, qs_sample_intra3, qs_sample4, qs_sample5, qs_sample6]
...
# Plot the end-effector paths for each trajectory separately (not printing the intra paths betweens digits)
...
plot_end_effector_path(qs_sample9, 'r')  # Path for trajectory 9
